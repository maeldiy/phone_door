...
*/
 ...
 //               Log.v("**Wakelock already held");
  ...
  //      SharedPreferences mPrefs = PreferenceManager.getDefaultSharedPreferences(mContext); // not used ?
        ...
        // mael update : as SCREEN_BRIGHT_WAKE_LOCK is deprecated, if state removed to set only SCREEN_DIM_WAKE_LOCK
        ...
        */
        ...
        // mael update : settings not required (and not managed ;-(, thus screen should always be turned on
 ...
 //       mWakeLock = mPm.newWakeLock(flags, Log.LOGTAG + ".full");
        mWakeLock = mPm.newWakeLock(flags,".full");
        mWakeLock.setReferenceCounted(false);
        mWakeLock.acquire();
       
//		if (BuildConfig.DEBUG)  //mael debug if removed
   //         Log.v("**Wakelock acquired");

        // Fetch wakelock/screen timeout from preferences
        // mael update settings not managed ;-(, thus removed
   
       /* int timeout = 1000; Integer.valueOf(mPrefs.getString(
                mContext.getString(R.string.pref_timeout_key), PREFS_TIMEOUT_DEFAULT));
*/
        // Set a receiver to remove all locks in "timeout" seconds
			// mael update : removed, tus all if removed
//        ClearAllReceiver.setCancel(mContext, timeout);
    }

    
    public static synchronized void acquirefull_screenoff(Context mContext) {  // this to maintain cpu alive and provide feedback to udp frame while screen is off
        if (mWakeLock != null) {
            if (BuildConfig.DEBUG)
 //               Log.v("**Wakelock already held");
            return;
 ...
 //       mWakeLock = mPm.newWakeLock(flags, Log.LOGTAG + ".full");
        mWakeLock = mPm.newWakeLock(flags,".fullscreenoff");
        mWakeLock.setReferenceCounted(false);
        mWakeLock.acquire();
...
*/
        // Set a receiver to remove all locks in "timeout" seconds
			// mael update : removed, thus all if statement removed
//        ClearAllReceiver.setCancel(mContext, timeout);
    }
    ...
    public static synchronized void acquirePartial(Context mContext) {
      ...
      //          mPm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, Log.LOGTAG + ".partial");
        		mPm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK,  ".partial");
  /*      if (BuildConfig.DEBUG)
